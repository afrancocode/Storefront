...
namespace Agathas.Storefront.Controllers.Controllers
{
	public class CheckoutController : BaseController
	{
		...
		{
			GetCustomerRequest customerRequest = new GetCustomerRequest() { CustomerIdentityToken = _formsAuthentication.GetAuthenticationToken() };
		...
		public ActionResult PlaceOrder(FormCollection collection)
		{
			...
			_cookieStorageService.Save(CookieDataKeys.BasketItems.ToString(), "0", DateTime.Now.AddDays(1)); _cookieStorageService.Save(CookieDataKeys.BasketTotal.ToString(), "0", DateTime.Now.AddDays(1));
